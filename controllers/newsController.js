...
const createNewsHandler = async (req, res) => {
  ...
  const image_content = req?.file?.filename ?? ''; // Multer adds 'file' object to 'req' containing uploaded file details
  try {
    const newNews = await News.create({ title, category_id, content, image_content, created_by: req.user.name });
